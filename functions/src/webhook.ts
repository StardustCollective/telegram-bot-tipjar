...
export class Webhook {
     ...
     * @param {string} userLanguage - user's telegram language
     * @return {Promise}
     */
    async handleStart(
        userId: string,
    ...
    */
     ...
     * @param {string} userLanguage - user's telegram language
     * @param {string} groupLanguage - group's set language
     * @return {Promise}
     */
    async handleTip(
        userId: string,
      ...
      if (amount > balance) {
        return Telegram.getInstance().sendText(
            userId,
            Language.getString(userLanguage, "tip.insufficient_balance"),
            Language.getKeyboard(userLanguage, this.INLINE_KEYBOARD)
        );
      }
    ...
    _getAvailableLanguagesFromFiles() {
      return readdirSync(this.LANGUAGES_FOLDER_PATH).map(
     ...
     * @param {string} targetLanguage - language to be set
     * @param {string} groupLanguage - group's set language
     * @return {Promise}
     */
    async handleSetLanguage(
      ...
      const chatMember = await Telegram.getInstance()
          .getGroupMemberData(chatId, userId);
     ...
     * @param {string} command command used to call default case
    ...
    async handleDefault(
      ...
      if (input.startsWith("/setlanguage")) {
        ...
        if (!targetLanguage) {
          return Telegram.getInstance().sendText(
              chatId,
              Language.getString(groupLanguage, "language.invalid_format"));
        }
      ...
      if (state.path === "withdrawal") {
        ...
        if (withdrawalState.section === "amount") {
          ...
          if (
            ...
            return Telegram.getInstance().sendText(
                userId,
                Language.getString(userLanguage, "withdrawal.invalid_amount")
            );
          ...
          return Telegram.getInstance().sendText(
              userId,
              Language.getString(userLanguage, "withdrawal.destination_text")
          );
          ...
          return Telegram.getInstance().sendText(
              ...
              Language.getString(
                  userLanguage,
                  "withdrawal.confirm_text",
                  tokens
              ),
              {inline: true, keys: [
                this._addKeyboardButton(
                    userLanguage,
                    "buttons.withdraw.confirm"
                ),
                this._addKeyboardButton(
                    userLanguage,
                    "buttons.withdraw.decline"
                ),
              ]}
          );
        }
    ...
    async handleCallbackQuery(
      ...
      if (section === "tip") {
        if (subject === "confirm") {
          ...
          const sourceGroupLanguage = await Database.getInstance()
              .getGroupLanguage(state.sourceChatId);
          await Telegram.getInstance().sendText(
              state.sourceChatId,
              Language.getString(
                  sourceGroupLanguage,
      ...
      if (section === "disclaimer") {
        if (subject === "accept") {
          ...
          return Telegram.getInstance().editMessage(
              chatId,
              messageId,
              Language.getString(userLanguage, "disclaimer.finished")
          );
        } else {
          return Telegram.getInstance().editMessage(
              chatId,
              messageId,
              Language.getString(userLanguage, "disclaimer.declined")
          );
        }
      }
      ...
      return Telegram.getInstance().editMessage(
          chatId,
          messageId,
          Language.getString(userLanguage, `${section}.${subject}`),
          {inline: true, keys: [
