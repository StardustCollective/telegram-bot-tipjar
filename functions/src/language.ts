...
export class Language {
     ...
     *
     * @param {string} language the language to pick
     * @param {boolean} inline should the keyboard be alongside the message
     * @return {TelegramKeyboard} the translated keyboard
     */
    static getKeyboard(
    ...
    getKeyboard(
      ...
      const {
        ...
        translatedDeposit,
        translatedWithdraw,
      }: TranslatedKeyboardStrings = currentInstance
          .getKeyboardTranslatedStrings(language);
    ...
    getKeyboardTranslatedStrings(language: string): TranslatedKeyboardStrings {
      const translatedBalance = currentInstance
          .reduceString(language, "keyboard.balance");
      const translatedHelp = currentInstance
          .reduceString(language, "keyboard.help");
      const translatedDeposit = currentInstance
          .reduceString(language, "keyboard.deposit");
      const translatedWithdraw = currentInstance
          .reduceString(language, "keyboard.withdraw");
